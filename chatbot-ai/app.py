...
def chat():
    ...
    try:
        ...
        session_histories[session_id].add_user_message(user_message)
        ...
        if qa_chain:
            try:
                ...
                # Extract customer info and generate pitch
                customer_data = extract_customer_info(session_id, user_message)
                pitch = generate_rule_based_pitch(customer_data) if customer_data else ""
                
                # Update Google Sheets if we have customer data
                if customer_data and setup_google_sheets():
            ...
            # Fallback to simple response if initialization failed
            response = get_simple_response(user_message)
            return jsonify({"message": response})
